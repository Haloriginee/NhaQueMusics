$(document).ready(function() {
	(function($){
		...
		if (pathname.indexOf("#artist=") >= 0){ // Check if artist
			...
			document.title = artistName + ' | Nha Que Musics';
			...
			// new page title / artist - song name
			document.title = genreName + ' | Nha Que Musics';
			loadGenre(genreName);
		}
	})(jQuery);
...
$.fn.yt = function(options){
	...
	function isLoaded () {
		if(videos.length > 0){
			...
			var newTitle = imeBenda + ' - ' + imePesme + ' | Nha Que Musics'
...
$.fn.lfmInfo = function(options){
	...
	function isLoaded (artistElement) {
		if (postojanje){
			...
			window.history.pushState("", "", "index.html#artist=" + imeBenda);
...
$.fn.lfmTagInfo = function(options){
	...
	function isLoaded (artistElement) {
		if (postojanje){
			stuffHappeningWhenLoadingArtistOrGenre();
			window.history.pushState("", "", "index.html#tag=" + imetaga);
			setTimeout(function(){
